...
impl Regex<char> {
   ...
   */
  fn new(input: &str) -> Result<Regex<char>, &'static str> {
    ...
    while let Some(frg) = fragments.next() {
      ...
      result = if frg.matches(is_identifier).next().is_some() && 1 < frg.len() {
        Regex::Concat(Box::new(result), Box::new(Regex::new(frg)?))
   ...
   */
  fn to_regex(identifier: char) -> Regex<char> {
    match identifier {
      '!' => Regex::Empty,
      '_' => Regex::All,
      _ => unreachable!(),
...
impl Recognizable<&str> for Regex<char> {
