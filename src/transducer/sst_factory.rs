...
pub fn replace_all<'a>(old: String, new: String) -> SymSST<Lambda<Predicate<char>>> {
  if old.len() == 0 {
    panic!("Invalid Input");
  }

  let accumulator = Rc::new(Variable::new());
  let result = Rc::new(Variable::new());
  ...
  let acc_alpha = Rc::new(vec![
    ...
    UpdateComp::F(Lambda::<Predicate<char>>::identity()),
  ]);
    ...
    if i == 1 {
      if let Some((_, p1, not_p1)) = &first {
        ...
        gamma.insert(
          ...
          Rc::new(vec![UpdateComp::F(Lambda::<Predicate<char>>::identity())]),
        );
        gamma.insert(
          Rc::clone(&result),
          Rc::new(vec![
  ...
  if let Some((first, p, not_p)) = first {
    let last_acc_alpha = Rc::new(vec![UpdateComp::F(Lambda::<Predicate<char>>::identity())]);
    let last_res_alpha = Rc::new({
      let mut v1 = vec![UpdateComp::X(Rc::clone(&result))];
      v1.extend(
        new
          .chars()
          .map(|c| UpdateComp::F(Lambda::<Predicate<char>>::constant(c))),
      );
      v1
    });
...
mod tests {
  ...
  fn abcxyz_to_1_sst_test() {
    ...
    assert_eq!(
      String::from_iter(&rep.run(&"aaaabcxyzzzzabcxyz".chars().collect::<Vec<char>>()[..])[0]),
      "aaa1zzz1".to_string()
    );
  }
}
